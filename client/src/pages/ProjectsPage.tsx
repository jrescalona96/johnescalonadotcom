    ...
    <>
      <div className="pt-[120px] pb-12">
        <Container>
          ...
          <h1 className="font-display text-[clamp(32px,4vw,48px)] font-bold leading-[1.1] tracking-tight">
            Projects
          </h1>
          <p className="mt-2 max-w-[560px] text-[18px] text-muted">
...
function ProjectCard({ project }: { project: Project }) {
  ...
  return (
    <div className="rounded-content border border-border bg-surface p-8 transition-colors duration-150 hover:border-accent-soft">
      ...
      {project.techStack && (
        <div className="mt-1 font-mono text-[12px] leading-[1.4] text-muted">
          <FontAwesomeIcon icon={faArrowUpRightFromSquare} className="mr-1 text-accent" />
          {project.techStack.join(" · ")}
        </div>
      )}
      {project.description?.[0] && (
        <p className="mt-[10px] max-w-[680px] text-[15px] leading-relaxed text-muted">
          {project.description[0]}
        </p>
      )}
      {project.tags && project.tags.length > 0 && (
        <div className="mt-[14px] flex flex-wrap gap-[6px]">
          {project.tags.map((tag) => (
            <span
              key={tag}
              className="rounded-sm bg-background px-[10px] py-[3px] font-mono text-[12px] leading-[1.4] text-muted"
            >
              {tag}
            </span>
          ))}
        ...
        <div className="mt-[14px] flex gap-4">
          {project.links.map((link) => (
            <a
              ...
              target="_blank"
              rel="noopener"
              className="font-body text-[14px] font-medium leading-none text-accent hover:underline"
            >
